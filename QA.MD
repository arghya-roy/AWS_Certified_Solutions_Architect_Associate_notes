...
## A company requires a fully managed replacement for an on-premises storage service. The company’s employees often work remotely from various locations. The solution should also be easily accessible to systems connected to the on-premises environment. Which solution meets these requirements?
...
- "Use Amazon FSx for Windows File Server for the Windows instances and the Linux instances" is the correct answer.
- Because we have to use same file server which will be accessed from windows and linux.

## A company has created an application that stores sales performance data in an Amazon DynamoDB table. A web application is being created to display the data. A Solutions Architect must design the web application using managed services that require minimal operational maintenance. Which architectures meet these requirements? (Select TWO.)
...
## A company has a Production VPC and a Pre-Production VPC. The Production VPC uses VPNs through a customer gateway to connect to a single device in an on-premises data center. The Pre-Production VPC uses a virtual private gateway attached to two AWS Direct Connect (DX) connections. Both VPCs are connected using a single VPC peering connection. How can a Solutions Architect improve this architecture to remove any single point of failure?
...
- "Increase the number of read transactions per shard" is incorrect. You cannot increase the number of read transactions per shard.
...
## Aurora global database provide - This provides your application with an effective Recovery Point Objective (RPO) of 1 second and a Recovery Time Objective (RTO) of less than 1 minute
## Aurora read replica provide - RTO objectives as large databases may well take more than 10 minutes to promote.

## For the fargate ecs storage we can attach only EFS and FSx for Windows File Server, Can not add EBS, Fsx for lustre
...
- "Update the application to read from the Aurora Replica" is the correct answer.
- No need to create any read replica because the aurora is multi az means we can your that as read replica.
...
- "Create a new trail in CloudTrail from within the management account with the organization trails option enabled" is the correct answer.
...
## RAID 1 is mirroring and does not increase the amount of IOPS you can generate.

## The processed data would be going from the EC2 instance to a bucket in a different Region which would incur data transfer charges.

## A Solutions Architect is creating a URL that lets users who sign in to the organization’s network securely access the AWS Management Console. The URL will include a sign-in token that authenticates the user to AWS. Microsoft Active Directory Federation Services is being used as the identity provider (IdP). Which of the steps below will the Solutions Architect need to include when developing the custom identity broker? (choose 2)
...
![image](https://user-images.githubusercontent.com/95282173/186900490-d722a469-560e-4ea5-b3e1-919c5ead3471.png)
...
- SAML is used for single sign-on between on-premises directories and the AWS management console.

## Cloudwatch basic monitoring - 5 min period
## cloudwatch detail monitoring - 1 min period
...
- The EBS Data Lifecycle Manager (DLM) can automate the creation, retention, and deletion of backups for the Amazon EBS volumes in an Amazon VPC for you and this can be performed centrally from within the management console.
...
## Elasticache can give sub-milisecond response but read replica can not.

## Aurora, RDS scaling can cause downtime but dynamodb can not.
...
- **Connection Draining** - Connection draining is enabled by default and provides a period of time for existing connections to close cleanly. When connection draining is in action an CLB will be in the status “InService: Instance deregistration currently in progress”.
- **Sticky Sessions** -  Session stickiness uses cookies and ensures a client is bound to an individual back-end instance for the duration of the cookie lifetime.
- **Proxy Protocol** - The Proxy Protocol header helps you identify the IP address of a client when you have a load balancer that uses TCP for back-end connections.
- **Deletion Protection** - Deletion protection is used to protect the ELB from deletion.
...
## default NACL has 
-    There is a default inbound rule allowing all traffic
-    There is a default outbound rule allowing all traffic
## a new Network ACL has 
...
### Pilot light -
 With a pilot light strategy a core minimum of services are running and the remainder are only brought online during a disaster recovery situation.
...
## Using cloudwatch we can lambda track of 
- the number of requests, 
- and the number of requests resulting in an error. 
